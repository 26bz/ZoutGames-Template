import { Check, X, Server, Globe, Mail, ShieldCheck, Database, Code } from 'lucide-react'
import { GlowingButton } from '../components/ui/GlowingButton'
import FAQ from '../components/FAQ'
import { Helmet } from 'react-helmet'

const PlanCard = ({ name, price, features, popular, href }) => (
  <div className={`p-4 sm:p-6 rounded-lg border ${popular ? 'border-yellow-400' : 'border-gray-800'} bg-slate-900 flex flex-col h-full`}>
    {popular && <span className="bg-yellow-400 ml-auto text-black text-xs font-bold px-3 py-1 rounded-full w-fit mb-4">Most Popular</span>}
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-slate-800">
            <th className="p-3 sm:p-4 text-left text-sm sm:text-base">Feature</th>
            {plans.map(plan => (
              <th key={plan.name} className="p-3 sm:p-4 text-center text-sm sm:text-base">
                {plan.name}
              </th>
            ))}
          </tr>
              ...
              {plans.map(plan => (
                <td key={`${plan.name}-${feature.name}`} className="p-3 sm:p-4 text-center">
                  {feature.values[plan.name] === true ? (
                    ...
                    <span className="text-gray-300 text-sm sm:text-base">{feature.values[plan.name]}</span>
                  )}
                </td>
              ))}
            </tr>
...
const CpanelHostingPage = () => {
  const plans = [
    {
      ...
      price: 4.99,
      popular: false,
      features: ['1 Website', '10GB SSD Storage', 'Unmetered Bandwidth', 'Free SSL Certificate', '5 Email Accounts'],
    ...
    },
    {
      ...
      price: 9.99,
      popular: true,
      features: ['10 Websites', '30GB SSD Storage', 'Unmetered Bandwidth', 'Free SSL Certificate', '25 Email Accounts', 'Domain Privacy'],
      href: 'http://example.com',
    },
    {
      name: 'Pro',
      price: 14.99,
      popular: false,
      features: ['Unlimited Websites', '100GB SSD Storage', 'Unmetered Bandwidth', 'Free SSL Certificate', 'Unlimited Email Accounts', 'Domain Privacy', 'Dedicated IP'],
      href: 'http://example.com',
    },
  ]
  ...
  const features = [
    {
      ...
      description: 'Industry-standard control panel for seamless website management',
    },
    {
      icon: Globe,
      title: '99.9% Uptime',
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="container max-w-screen-xl mx-auto px-4 sm:px-6 py-16 sm:py-32">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <div className="max-w-xl">
              <span className="text-yellow-400 text-sm font-medium mb-2 block">Web Hosting</span>
              ...
              <p className="text-gray-400 text-base sm:text-lg mb-6 sm:mb-8">
                Power your website with lightning-fast SSD storage, free SSL certificates, and industry-leading cPanel control panel.
              </p>
            </div>
            <div className="relative h-[300px] lg:h-[500px] rounded-lg overflow-hidden">
              <img src="https://placehold.co/600x500" alt="cPanel Dashboard Preview" className="w-full h-full object-cover rounded-lg" />
      ...
      <div className="container max-w-screen-xl mx-auto px-4 sm:px-6 py-16 sm:py-32">
        <h2 className="text-2xl sm:text-3xl font-bold text-center mb-8 sm:mb-12">Choose Your Hosting Plan</h2>
