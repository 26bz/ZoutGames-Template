...
const GameShowcase = () => {
  const games = [
    ...
    },
    {
      id: 4,
      name: 'ARK: SE',
      image: 'https://external-preview.redd.it/CAA9sdPu0Moxk1jLfnMjiaFTQhBmB7uP7gGE7myUadY.jpg?auto=webp&s=9b35cde1a9af2e771ce33182e2099063b773bda1',
    },
  ]
  ...
  return (
    <div className="relative h-full flex items-center">
      {games.map((game, index) => (
        <GameCard key={game.id} game={game} index={index} total={games.length} />
      ))}
    </div>
  )
}
...
const FeatureTag = ({ text, delay }) => (
  <motion.span initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay }} className="px-3 py-1 rounded-full bg-slate-900 text-gray-300 text-sm">
    {text}
  </motion.span>
)
        ...
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-4">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold">
              <span className="bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">You Create the Gaming Experience</span>
              <span className="text-yellow-400">.</span>
            </h1>

            <p className="text-lg sm:text-xl text-gray-300 leading-relaxed">
              New to Gaming? or Have you been Gaming for years? Regardless of your experience you will feel the power of our game servers!
            </p>

            <div className="flex flex-wrap gap-3">
              {['Instant Setup', '24/7 Support', 'DDoS Protection'].map((tag, index) => (
            ...
            <div className="pt-2">
              <Link to="/gaming-servers">
                <GlowingButton className="text-black py-3 px-4">Game Servers $4/month</GlowingButton>
              </Link>
            </div>
          </motion.div>
